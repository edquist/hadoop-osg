...
for component in $HADOOP_DIR/bin/hadoop $HDFS_DIR/bin/hdfs $YARN_DIR/bin/yarn ; do
...
chmod 644 ${HADOOP_DIR}/*.jar ${HDFS_DIR}/*.jar ${YARN_DIR}/*.jar
...
for DIR in ${HADOOP_DIR} ${HDFS_DIR} ${YARN_DIR} ${HTTPFS_DIR} ; do
  (cd $DIR &&
   rm -fv *-sources.jar
...
for file in `cat ${BUILD_DIR}/hadoop-client.list` ; do
  for dir in ${HADOOP_DIR}/{lib,} ${HDFS_DIR}/{lib,} ${YARN_DIR}/{lib,} ; do
    [ -e $dir/$file ] && \
